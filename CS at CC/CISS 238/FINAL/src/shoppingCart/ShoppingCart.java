...
public class ShoppingCart extends Application
{
   ...
   @Override
   	...
   	{
	   try {//open a file, print message if it does not exist
	   readBookFile();
	   }
      ...
      Label subTotalValue = new Label("" + sub);
      ...
      addToCartButton.setOnAction(e ->
      {
         ...
         if (index != -1)
         {
            ...
            cartPrices[cartIndex] = bookPrices[index];
            cartIndex++;
            // Update the subtotal
            sub += bookPrices[index];
            
            // Update the pricing label on the GUI
            subTotalValue.setText(String.format("%.2f", sub));
      ...
      {
         // Get the selected index.
    	  cartIndex = cartView.getSelectionModel().getSelectedIndex();
         ...
         if (index != -1)
         {
           */ // Update the subtotal
            ...
            cartList.remove(cartIndex);//update the cartPrices array to match the listView
            while(cartPrices[cartIndex] != 0 && cartIndex < cartPrices.length) {
            	cartPrices[cartIndex] = cartPrices[cartIndex + 1];
            	cartIndex++;
            }
      ...
      checkoutButton.setOnAction(e ->
      {
         ...
         try {
			   ...
			   //create the receipt with formatting
			   receiptFile.println("Receipt: " + timeStamp);
			   receiptFile.println("Title				Price");
			   receiptFile.println("-----------------		-----");
			   
			   int x = 0;
			   while(x < cartList.size()) {//print the full list of items in the cart
      ...
      VBox priceLabels = new VBox(subTotalLabel);
      VBox priceVals = new VBox(subTotalValue);
      HBox priceCombo = new HBox(10, priceLabels, priceVals);
      // Put everything into a VBox
      VBox layout = new VBox(20, titles, lists, priceCombo);
   ...
   private void readBookFile() throws IOException
   {
      ...
      while (inFile.hasNext())
      {
         ...
         // Add the book info to the Arrays
